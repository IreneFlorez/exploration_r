...
t0 = plot.hist('alcohol', df, 100)  
t1 = plot.hist('alcohol', df, 100) + 
  scale_x_continuous(limits = c(4, 12), breaks = seq(4, 12, 1))
grid.arrange(t0, t1)
```

```{r echo=FALSE, alcohol5}
#alcohol grid: histogram & box plot
...
#sulphates distribtion views & boxplot
...
#sulphate histogram and scatterboxplot
...
The first histogram is shifted to the left, which means density variable has at least one outlier. 
...
ggplot(aes(x = residual.sugar), data = df) + geom_histogram(binwidth = 0.01)              
```

```{r, echo=FALSE, residual_sugar3}
#residual sugar histogram views and scatter boxplot
s0 = plot.hist('residual.sugar', df, 30)
s1 =  ggplot(df, aes(y = residual.sugar, x = 1)) + 
  geom_jitter(aes(x = 1, y = residual.sugar), alpha = 0.15) +
  geom_boxplot(color = 'blue', alpha = 1/4) + coord_flip()
s2 = plot.hist('residual.sugar', df, 75) 
grid.arrange(s0, s1, s2)
```
The majority of white wines in the dataset have a low residual sugar level. 
...
Generally, as quality increases, sulphate content increases; but decreases when it 
reaches a 7.5 quality score.  
